...
while [ ${instance} -lt ${num_benchmarks} ]; do
    echo "====================================================="
    echo "Benchmark Instance:" ${instance}
    instance_output_path=${output_path}/${instance}
    echo "  Benchmark result path:" ${instance_output_path}
    mkdir -p ${instance_output_path}
    ${SCRIPT_DIR}/incremental_reconstruction.sh -d ${dataset_path}/${instance} -o ${instance_output_path} -i ${increment} -n 10
    instance=$(($instance + 1))
done
